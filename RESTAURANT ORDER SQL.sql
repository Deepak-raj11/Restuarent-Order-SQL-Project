...
-- Rank menu items by total sales within each category:
WITH mycte AS (SELECT 
    menu_item_id,  
    category,
    item_name, 
    SUM(price) AS total_sold,
    RANK() OVER (PARTITION BY category ORDER BY SUM(price) DESC) AS rank_in_category
FROM order_details
LEFT JOIN menu_items
ON order_details.item_id = menu_items.menu_item_id
GROUP BY menu_item_id, item_name, category;)
SELECT rank_in_category FROM mycte
...
on od.order_id =c.member_id

SELECT first_name,SUM(price) as price
FROM order_details od
JOIN menu_items mi
ON od.item_id = mi.menu_item_id
JOIN customers c
